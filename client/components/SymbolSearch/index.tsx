...
const SymbolSearch = ({ callback }: SymbolSearchProps) => {
  ...
  const errorHandler = (errorMsg = 'Unable to Retrieve Tickers') => {
    setErrorMsg(errorMsg);
    setResults([]);
  };

  const searchSymbols = async () => {
    const validTickerRegex = /^[A-Za-z]+/;
    if (
      !searchQuery ||
      searchQuery.trim() === '' ||
      !validTickerRegex.test(searchQuery)
    ) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (!data) errorHandler();
          ...
          errorHandler(`No Tickers found for ${searchQuery}`);
